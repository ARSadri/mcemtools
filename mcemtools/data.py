...
def segmented_detector_maker(
    ...
    If `rings_radii_ranges` is not provided, it will be generated by dividing
    the radial range [inner_radius, outer_radius] into equal-width rings.
    ...
    rings_radii_ranges : list of [float, float], optional
        Explicit list of inner and outer radius pairs for each ring.
    outer_radius : float, optional
        Maximum radius. Used only if rings_radii_ranges is not provided.
    inner_radius : float, optional
        Starting radius. Defaults to 0.
    ring_gap_angles : list of float, optional
        Total angular gap (in degrees) per ring.
    centre : tuple of float, optional
        (x, y) coordinates for the detector center.
    ...
    -------
    masks : np.ndarray
    det_segs_ringid : np.ndarray
    """

    # -------------------------
    # Validate mutually exclusive inputs
    # -------------------------
    assert not (rings_radii_ranges is not None and outer_radius is not None), \
        "Provide either rings_radii_ranges OR outer_radius, not both"

    assert (rings_radii_ranges is not None) or (outer_radius is not None), \
        "You must provide either rings_radii_ranges or outer_radius"

    n_rings = len(rings_num_segments)

    # -------------------------
    # Auto-generate radii ranges
    # -------------------------
    if rings_radii_ranges is None:
        assert outer_radius > inner_radius, \
            "outer_radius must be greater than inner_radius"

        edges = np.linspace(inner_radius, outer_radius, n_rings + 1)
        rings_radii_ranges = [
            [edges[i], edges[i + 1]] for i in range(n_rings)
        ]

    else:
        assert len(rings_radii_ranges) == n_rings, \
            "rings_radii_ranges must match rings_num_segments length"
    ...
    if ring_gap_angles is None:
        ...
        assert len(ring_gap_angles) == n_rings, \
            "ring_gap_angles must match number of rings"
